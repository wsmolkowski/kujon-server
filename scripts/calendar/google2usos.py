...
parser = argparse.ArgumentParser(
    description="""Simple program using USOS API and Google Calendar Data API to copy your sked to google. See http://winemore.w.staszic.waw.pl/usos2google.html for more info.""")
...
parser.add_argument('-p', '--password',
                    help="Your google account password (do not use this option if you don't have to, program will ask for your password!)")
...
if not empty(access_token_key):
    ...
    print
    "%s?oauth_token=%s" % (authorize_url, request_token.key)
    print
    ...
    print
    "access_token_key = '%s'" % access_token.key
    print
    "access_token_secret = '%s'" % access_token.secret
...
print
"Select calendar to copy sked to:"
cals = []
alink = None
for i, cal in enumerate(feed.entry):
    print
    ' %s. %s' % (i, cal.title.text)
    cals += [cal]
i = int(i) + 1
print
" %d. Create new calendar" % i
...
print
"Copying events, this may take a while."

for i in xrange(total):
    sys.stdout.write("%d. week: " % (i + 1))
    resp, content = usosc.request(
        usosapi_base_url + "services/tt/student?start=%s&days=7&fields=name|start_time|end_time|classtype_name|classtype_id|course_name|course_id|building_id|building_name|room_number|classgroup_profile_url" % day.isoformat(),
        "GET")
    if resp['status'] != '200':
    ...
    for i in items:
        ...
        event.where.append(
            gdata.calendar.data.CalendarWhere(value=("%s %s" % (i['room_number'], i['building_name']['pl']))))
