...
class Dao:
    ...
    def get_user_jobs(self, status):
        if constants.COLLECTION_JOBS_INITIAL_USER not in self._db.collection_names():
            return list()

        return self._db[constants.COLLECTION_JOBS_INITIAL_USER].find({constants.JOB_STATUS: status},
                                                                     (constants.USER_ID,))

    def update_user_job(self, job_id, status):
        update = {"$set": {constants.JOB_STATUS: status, constants.UPDATE_TIME: datetime.now()}}

        result = self._db[constants.COLLECTION_JOBS_INITIAL_USER].update({constants.ID: job_id}, update)

        logging.debug(u"update_user_job result {0}".format(result))

    def insert_user_job(self, user_id):
        return self._db[constants.COLLECTION_JOBS_INITIAL_USER].insert(mongo_utils.user_job_insert(user_id))
    ...
    def get_user_jobs_for_update(self):
        ...
        docs = self._db[constants.COLLECTION_JOBS_INITIAL_USER].find(
