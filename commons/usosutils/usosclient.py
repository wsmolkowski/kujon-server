...
class UsosClient:
    ...
    def user_info(self, user_info_id):
        if user_info_id:
            code, body = self.client.request(u"{0}{1}{2}{3}".format(self.base_url, URI_USER_INFO, u"&user_id=", user_info_id))
        else:
    ...
    def user_info_photo(self, user_id):
        ...
        if code['status'] == '200':
            ...
            return result
        raise Exception(u"Error while fetching photo for request: {0}. Response code: {1} body: {2}".format(request, code, body))
    ...
    def programme(self, programme_id):
        if programme_id:
            ...
            if code['status'] == '200':
                return json.loads(body)
            raise Exception(u"Error while fetching programmes for requesst: {0}. Response code: {1} body: {2}".format(request, code, body))
    ...
    def groups(self, course_unit_id):
        ...
        if code['status'] == '200':
            return json.loads(body)
        raise Exception(u"Error while fetching groups for request: {0}. Response code: {1} body: {2}".format(request, code, body))
    ...
    def units(self, unit_id):
        ...
        if code['status'] == '200':
            return json.loads(body)
        raise Exception(u"Error while fetching units for request {0}. Response code: {1} body: {2}".format(request, code, body))
    ...
    def courseeditions_info(self):
        ...
        if code['status'] == '200':
            return json.loads(body)
        raise Exception(u"Error while fetching courseeditions_info for request: {0}. Response code: {1} body: {2}".format(request, code, body))
    ...
    def course_edition(self, course_id, term_id, fetch_participants):
        ...
        if code['status'] == '200':
            return json.loads(body)
        raise Exception(u"Error while fetching course_info for request: {0}. Response code: {1} body: {2}".format(request, course_id, code, body))
    ...
    def grades(self, course_id, term_id):
        ...
        if code['status'] == '200':
            return json.loads(body)
        raise Exception(u"Error while fetching grades for request: {0}. Response code: {1} body: {2}".format(request, code, body))
    ...
    def class_types(self):
        ...
        if code['status'] == '200':
            return json.loads(body)
        raise Exception(u"Error while fetching class_types for request {0}. Response code: {1} body: {2}".format(request, code, body))
