...
class LoginHandler(BaseHandler):
    ...
    def post(self):
        ...
        if user_doc:
            self.set_secure_cookie(constants.USER_SECURE_COOKIE, str(user_doc)) #json_util.dumps(
        ...
        if user_doc:
            ...
            try:
                ...
                data[constants.ALERT_MESSAGE] = "failed user_doc authenticate with {0} {1}".format(
                            updated_user[constants.ACCESS_TOKEN_SECRET], updated_user[constants.ACCESS_TOKEN_KEY])

            self.render("login.html", **data)
        else:

            data[constants.ALERT_MESSAGE] = "user_doc not found for given oauth_token_key:{0}, oauth_verifier: {1}".format(oauth_token_key, oauth_verifier)
