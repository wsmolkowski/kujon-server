...
def _entry_disposition(response_headers, request_headers):
    ...
    since we operate as if every usosapi has sent 'must-revalidate'.
...
class HmacDigestAuthentication(Authentication):
    ...
    def __init__(self, credentials, host, request_uri, headers, response, content, http):
        ...
        if not self.challenge.get('snonce'):
            raise UnimplementedHmacDigestAuthOptionError( _("The challenge doesn't contain a usosapi nonce, or this one is empty."))
...
class WsseAuthentication(Authentication):
    ...
    At this time there isn't any third party usosapi to test against.
    ...
    def _conn_request(self, conn, request_uri, method, body, headers):
        for i in range(2):
            try:
                ...
                conn.close()
                raise ServerNotFoundError("Unable to find the usosapi at %s" % conn.host)
            except (socket.error, httplib.HTTPException):
                # Just because the usosapi closed the connection doesn't apparently mean
                # that the usosapi didn't send a response.
                pass
            try:
...
class Response(dict):
    ...
    """HTTP protocol version used by usosapi. 10 for HTTP/1.0, 11 for HTTP/1.1. """
    version = 11

    "Status code returned by usosapi. "
    status = 200

    """Reason phrase returned by usosapi."""
    reason = "Ok"
