...
class ApiCoursesTest(TestBaseClassApp):
    ...
    def testShouldPresentCoursesForUser(self):
        ...
        self.assertTrue(
            '"course_id": "1000-612ARR", "term_id": "2004/TZ", "course_name": {"en": "Distributed and Parallel Algorithms", "pl": "Algorytmy rozproszone i r\u00f3wnoleg\u0142e"}}' in response.body)
        self.assertEqual(response.code, 200)

    def testShouldFailForWrongUser(self):
        response = self.fetch(
            "/api/courseseditions?mobile_id=FAKE&user_id=FAKE&usos={UW}&access_token_key=FAKE&access_token_secret=FAKE")
        self.assertEqual(response.code, 400)
